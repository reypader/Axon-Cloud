...
import org.axonframework.aws.SQSMessageSource;
import org.axonframework.aws.SQSPublisher;
...
public class AxonAWSAutoConfiguration {
    ...
    @Bean
    public SQSMessageConverter amqpMessageConverter(@Qualifier("eventSerializer") Serializer eventSerializer) {
    ...
    @ConditionalOnProperty("axon.aws.sns-topic-name")
    @ConditionalOnMissingBean
    @Bean(initMethod = "start", destroyMethod = "shutDown")
    public SQSPublisher amqpBridge(EventBus eventBus, AmazonSNS amazonSns, SQSMessageConverter sqsMessageConverter, AWSPProperties amqpProperties) {
        LOGGER.info("Initializing SQSPublisher");
        return new SQSPublisher(eventBus, amazonSns, sqsMessageConverter, amqpProperties);
    }
